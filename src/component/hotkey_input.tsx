...
export function HotkeyInput({value, onChange}: HotkeyInputProps) {
    ...
    return (
        <input
            ...
            onChange={() => {
            }} // Отключаем обычный ввод
            placeholder="Нажмите сочетание клавиш"
            className="mt-1 block w-full rounded border border-gray-300 bg-white dark:bg-gray-700 text-gray-900
            dark:text-gray-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
    );
}
